...
pub async fn send_email_gmail(
    ...
    let tcp_stream = TcpStream::connect(&addr)
        .await
        .map_err(|e| format!("failed to connect to SMTP server {}: {}", addr, e))?;
    let mut reader = BufReader::new(tcp_stream);

    // Read Greeting
    read_smtp_response_lines(&mut reader)
        .await
    ...
    read_smtp_response_lines(&mut reader)
        .await
        .map_err(|e| format!("failed to read EHLO response: {}", e))?;
    ...
    let config = rmod::rustls::ClientConfig::builder()
        .with_root_certificates(root_cert_store)
        .with_no_client_auth();
    let connector = tokio_rustls::TlsConnector::from(Arc::new(config));
    let server_name = rustls_pki_types::ServerName::try_from(host)
        .map_err(|e| format!("invalid host name for TLS: {}", e))?
        .to_owned();
    ...
    read_smtp_response_lines(&mut tls_reader)
        .await
        .map_err(|e| format!("failed to read TLS EHLO response: {}", e))?;
    ...
    read_smtp_response_lines(&mut tls_reader)
        .await
        .map_err(|e| format!("failed to read AUTH LOGIN response: {}", e))?;
    ...
    read_smtp_response_lines(&mut tls_reader)
        .await
        .map_err(|e| format!("failed to read username response: {}", e))?;

    // Send base64 Password
    let pass_b64 = STANDARD.encode(password);
    tls_reader.get_mut().write_all(format!("{}\r\n", pass_b64).as_bytes()).await.map_err(|e| format!("password write failed: {}", e))?;
    tls_reader.get_mut().flush().await.map_err(|e| format!("flush failed: {}", e))?;
    let pass_resp = read_smtp_response_lines(&mut tls_reader)
        .await
        .map_err(|e| format!("failed to read password response: {}", e))?;
    ...
    // MAIL FROM
    tls_reader.get_mut().write_all(format!("MAIL FROM:<{}>\r\n", username).as_bytes()).await.map_err(|e| format!("MAIL FROM write failed: {}", e))?;
    tls_reader.get_mut().flush().await.map_err(|e| format!("flush failed: {}", e))?;
    read_smtp_response_lines(&mut tls_reader)
        .await
        .map_err(|e| format!("failed to read MAIL FROM response: {}", e))?;
    ...
    for rcpt in recipients {
        tls_reader.get_mut().write_all(format!("RCPT TO:<{}>\r\n", rcpt).as_bytes()).await.map_err(|e| format!("RCPT TO write failed: {}", e))?;
        tls_reader.get_mut().flush().await.map_err(|e| format!("flush failed: {}", e))?;
        read_smtp_response_lines(&mut tls_reader)
            .await
            .map_err(|e| format!("failed to read RCPT TO response: {}", e))?;
    }
    ...
    read_smtp_response_lines(&mut tls_reader)
        .await
        .map_err(|e| format!("failed to read DATA response: {}", e))?;
    ...
    read_smtp_response_lines(&mut tls_reader)
        .await
        .map_err(|e| format!("failed to read body send response: {}", e))?;
